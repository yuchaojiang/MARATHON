...
CODEX2 [@jiang2018codex2] builds on CODEX with a significant improvement of sensitivity for both rare and common variants.
...
```{r fig.width=8, fig.height=4,echo=FALSE, out.width = "150%", dpi=200}
plotHMMscore(icnv_res0,title=projectname)
```

 Additional functionality of iCNV see [iCNV notebook](http://raw.githack.com/zhouzilu/iCNV/master/vignettes/iCNV-vignette.html).
 ...
 * We use UnifiedGenotyper by **[GATK](https://software.broadinstitute.org/gatk/)** to call somatic SNAs and follow its [Best Practices](https://software.broadinstitute.org/gatk/best-practices/). **[MuTect](http://archive.broadinstitute.org/cancer/cga/mutect)** and **[VarScan2](http://massgenomics.org/varscan)** can also be used when paired normal samples are available.
...
```{r, out.width = "600px", fig.align = "center", echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/yuchaojiang/MARATHON/master/instruction/demo-page-001.jpg")
```


#### Convert VCF file to input for Canopy
readVCFforCanopy is a function which generates canopy input from a VCF file. This function takes as input a the path to the VCF file.  First 9 column names after the header should be c("#CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT").  Following column names should be sample names. see \href{http://www.internationalgenome.org/wiki/Analysis/variant-call-format/}{VCF format}.  The function outputs a list of R(alternate reads), X(total reads), and vcfTargets which contains gene target info from the VCF.  
...
```{r, out.width = "600px", fig.align = "center", echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/yuchaojiang/MARATHON/master/instruction/primary.jpg")
```
Relapse genome Sequenza segementation genome-wide view
```{r, out.width = "600px", fig.align = "center", echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/yuchaojiang/MARATHON/master/instruction/relapse.jpg")
```
